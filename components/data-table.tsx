...
export function DataTable<TData, TValue>({
  ...
  const [selectedColumn, setSelectedColumn] = useState<string>(filterOptions[0]?.id || '')
  ...
  return (
    <div className="space-y-4">
      ...
      {filterOptions.length > 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-md border bg-gray-50 p-2 sm:flex-row">
          ...
          <div className="relative flex w-full items-center gap-2 sm:max-w-sm">
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <Loader2 className="h-5 w-5 animate-spin text-[#17375F]" />
                    Carregando dados...
                  </div>
                </TableCell>
              </TableRow>
                ...
                <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id}>
                      <div
                        className="max-w-[120px] truncate sm:max-w-[200px] md:max-w-[300px]"
                        title={
                          typeof cell.getValue() === 'string' ? String(cell.getValue()) : undefined
                        }
                      >
